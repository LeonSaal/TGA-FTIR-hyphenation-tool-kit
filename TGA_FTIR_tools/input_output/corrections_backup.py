...
def corr_FTIR(FTIR,file_baseline,plot=False):
    ...
    gases=['CO','CO2','H2O']
    ##return value
    for gas,thresh in zip(gases,[0.002,0.07,0.001]):
        corr_data[gas]=corr_data[gas].subtract(min(corr_data[gas]))
        corr_data[gas]=corr_data[gas].subtract(const_baseline(corr_data[gas],thresh))
