...
def TGA_info(file, TGA, profile="Otto"):
    ...
    footer = pd.read_table(
        path, encoding="ansi", skipfooter=2, index_col=False, names=[0], engine="python"
    ).tail(3)
    header = str(
        pd.read_table(
            path,
    ...
    info["method"] = method
    info["initial_mass"] = pd.to_numeric(
        re.search("(?<=\s)\S+(?=\smg)", footer.iloc[0, 0]).group().replace(",", ".")
    )  # str(footer).strip()
    ...
    info["mass_steps"] = [
        ...
        if parameters[index] == "method_gas"
    ]
    info["step_temp"] = [
        values[index]
        for index in range(len(parameters))
        if parameters[index] == "high_temp"
    ]
...
def dry_weight(
    ...
    # if how_dry is a number, the dry point is set to that temperature
        ...
        if (how_dry > max(TG_IR.tga["sample_temp"])) or (
            how_dry <= min(TG_IR.tga["sample_temp"])
        ):
            how_dry = None
            dry_point = 1
            print(
    ...
    # if how_dry is 'H2O' or 'sample_mass', the dry point is determined from the respective data
        ...
        if how_dry == "sample_mass":
            ...
            ref["sample_mass"] = -sp.signal.savgol_filter(
                TG_IR.tga["sample_mass"] / TG_IR.info["initial_mass"],
                WINDOW_LENGTH,
                POLYORDER,
                deriv=1,
            )
    ...
    if how_dry == None:
        info["reference_mass"] = "initial_mass"
        times = [0] + list(
            TG_IR.tga.index[TG_IR.tga["reference_temp"].isin(TG_IR.info["step_temp"])]
        )
    ...
    if plot:
        ...
        for i in range(len(times) - 1):
            ax.annotate(
                text="",
                xy=(x[times[i]], y[times[i]]),
                xytext=(x[times[i]], y[times[i + 1]]),
            ...
            ax.text(
                ...
                "$ML$ {}: {:.2f} mg ({:.1f} %)".format(
                    get_label(names[i]),
                    mass_loss[i],
                    mass_loss[i] / TG_IR.info[TG_IR.info["reference_mass"]] * 100,
                ),
            )
        ...
        if save:
            ...
            fig.savefig(
                os.path.join(
                    path_plots, "{}_mass_steps.png".format(TG_IR.info["name"])
                ),
                bbox_inches="tight",
                dpi=DPI,
            )
