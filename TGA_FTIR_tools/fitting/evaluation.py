...
def summarize(path, select_groups=[], condense_results=True, sort_samples=False):
    ...
    robustness_summary = pd.read_excel(
        path + "/robustness_in_mmol_per_mg.xlsx",
        sheet_name="summary",
        header=0,
        index_col=1,
    )
    ...
    robustness_summary = robustness_summary.set_index(
        ["samples", robustness_summary.index]
    )

    # Transpose the df, select relevant columns and rename them
    try:
        df = robustness_summary.T.loc[
            :, (slice(None), ["mean", "stddev", "limits"])
        ].rename(columns={"mean": "mmol_per_mg", "limits": "label"})
    except:
        df = robustness_summary.T.loc[:, (slice(None), ["mean", "stddev"])].rename(
            columns={"mean": "mmol_per_mg"}
        )
    ...
    if condense_results == True:
        ...
        groups = list(
            set([re.split("_", group)[0] for group in df.index if group not in gases])
        )
        ...
        for group in groups:
            ...
            if group == "anhydrides":
                summarized.loc[group, (slice(None), "mmol_per_mg")] = group_set.loc[
                    :, (slice(None), "mmol_per_mg")
                ].mean(axis=0)
                # error propagation for the mean of stddev
                group_set.loc[:, (slice(None), "stddev")] = (
                    group_set.loc[:, (slice(None), "stddev")] ** 2
                )
                summarized.loc[group, (slice(None), "stddev")] = group_set.loc[
                    :, (slice(None), "stddev")
                ].sum(axis=0) ** 0.5 / len(group_set.loc[:, (slice(None), "stddev")])
            else:
                summarized.loc[group, (slice(None), "mmol_per_mg")] = group_set.loc[
                    :, (slice(None), "mmol_per_mg")
                ].sum(axis=0)
                ...
                summarized.loc[group, (slice(None), "stddev")] = (
                    group_set.loc[:, (slice(None), "stddev")].sum(axis=0) ** 0.5
                )
            ...
            for sample in samples:
                if summarized.loc[group, (sample, "mmol_per_mg")] == 0.0:
                    try:
                        mask = pd.notnull(
                            group_set.loc[:, (sample, "label")]
                        )  # select rows with labels in group_set
                        label_set = set(
                            group_set.loc[mask, (sample, "label")]
                        )  # create a set of these labels
                        if len(label_set) == 2:  # if there are two items in the set
                            label_text = "< LOQ"
                        elif len(label_set) == 1:  # if there is only one item
                            label_text = "".join(
                                str(e) for e in label_set
                            )  # interesting syntax, but neseccary not to get "{...}"
                        summarized.loc[
                            group, (sample, "label")
                        ] = label_text  # set the label according to label_text
                    except:
                        pass
    else:
...
def concatenate(dfs, sample_order=[]):
    ...
    for sample in samples:
        try:
            df.loc[
                df.loc[:, (sample, "mmol_per_mg")].isnull(), (sample, "label")
            ] = "n.d."
