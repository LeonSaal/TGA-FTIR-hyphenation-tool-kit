...
def fitting(
    ...
    for gas in gases:
        ...
        if gas in gas_links:
            ...
            for group in df.index:
                ...
                for letter in df.loc[group, gas]:
                    ...
                    if param == "height":
                        preset = (
                            peaks.loc["{}_{}".format(group, other), param]
                    ...
                    temp_presets[gas].loc[group, "{}_min".format(param)] = preset * (
                        1 - predef_tol
                    )
                    temp_presets[gas].loc[group, "{}_max".format(param)] = preset * (
                        1 + predef_tol
                    )
        ...
        if plot:
            ...
            if title == True:
                fitting.set_title(
                    "{}, {} = {:.2f} ${}$".format(
                        TG_IR.info["alias"],
                        TG_IR.info["reference_mass"],
                        TG_IR.info[TG_IR.info["reference_mass"]],
                        UNITS["sample_mass"],
                    )
                )
        ...
        if plot:
            ...
            if y_axis == "orig":
                fitting.set_ylabel(
                    "{} {} ${}$".format(get_label(gas), SEP, UNITS["ir"])
                )
            elif y_axis == "rel":
                fitting.set_ylabel(
                    "{} {} ${}\,{}^{{-1}}\,{}^{{-1}}$".format(
            ...
            error.text(
                ...
                "        SQERR: {:.2e} ({:.2f}%)".format(
                    sumsqerr[gas][
                        TG_IR.info["name"]
                    ],  # ,'SQERR: '+'%.2E'% Decimal(sumsqerr[gas][TG_IR.info['name']]),va='bottom')
                    100 * sumsqerr[gas][TG_IR.info["name"]] / peaks[ir_values][gas],
                ),
            )  # percentage SQERR
            error.plot(x, diff)
            ...
            plt.show()
            fig.savefig(
                TG_IR.info["name"] + "_" + gas + ".png", bbox_inches="tight", dpi=DPI
            )
...
def fits(objs, reference, save=True, presets=None, **kwargs):
    ...
    if save:
        ...
        path = os.path.join(
            PATHS["dir_fitting"], time() + reference + "_" + "_" + sample_names
