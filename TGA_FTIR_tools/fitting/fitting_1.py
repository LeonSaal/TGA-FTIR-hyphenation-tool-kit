...
def fitting(
    ...
    peaks = pd.DataFrame(
        columns=["center", "height", "hwhm", "area", "mmol", "mmol_per_mg"],
        index=[group + "_" + gas for gas in gases for group in temp_presets[gas].index]
        + [gas for gas in gases],
    )

    # index= pd.MultiIndex.from_tuples([(group, gas) for gas in presets.keys() for group in presets[gas].index]+[('total', gas) for gas in presets.keys()], names=['group','gas'])
    # columns=["center", "height", "hwhm", "area", "mmol", "mmol_per_mg"]
    # pd.DataFrame(columns=columns, index=index).sort_index()
    ...
    for gas in gases:
        ...
        if "linreg" in TG_IR.__dict__:
            tot_mol = (tot_area - TG_IR.linreg["intercept"][gas]) / TG_IR.linreg[
                "slope"
            ][gas]
        ...
        # peaks.loc[('total', gas)] = tot_area

        if y_axis == "rel":
            FTIR.update(FTIR[gas] / tot_area * tot_mol)

        for key in ["_0", "_min", "_max"]:
            temp_presets[gas].loc[:, "height" + key] = (
                temp_presets[gas].loc[:, "height" + key].multiply(max(TG_IR.ir[gas]))
            )
        ...
        if gas in gas_links:
            ...
            for group in df.index:
                ...
                for letter in df.loc[group, gas]:
                    ...
                    if param == "height":
                        preset = (
                            peaks.loc[f"{group}_{other}", param]
                            / TG_IR.linreg["slope"][other]
                            * TG_IR.linreg["slope"][gas]
                        )
                    else:
                        preset = peaks.loc[f"{group}_{other}", param]
                    temp_presets[gas].loc[group, f"{param}_0"] = preset
                    temp_presets[gas].loc[group, f"{param}_min"] = preset * (
                        1 - predef_tol
                    )
                    temp_presets[gas].loc[group, f"{param}_max"] = preset * (
                        1 + predef_tol
                    )
        ...
        try:
            popt, _ = sp.optimize.curve_fit(
                func, x, FTIR[gas], p0=params_0, bounds=(params_min, params_max)
            )
        except:
        ...
        for i in range(num_curves):
            ...
            if y_axis == "orig":
                ...
                peaks["mmol"][group] = (
                    (peaks["area"][group] - TG_IR.linreg["intercept"][gas])
                    / TG_IR.linreg["slope"][gas]
                )  # calculation based on each Gauß fit
                peaks["mmol_per_mg"][group] = (
                    peaks["mmol"][group] / TG_IR.info[ref_mass]
                )
                ir_values = "area"
            elif y_axis == "rel":
                # peaks['mmol'][group]=peaks['area'][group]/tot_area*tot_mol   # calculation based relative to total evolved gas
        ...
        if plot:
            ...
            if title == True:
                fitting.set_title(
                    f"{TG_IR.info['alias']}, {TG_IR.info['reference_mass']} = {TG_IR.info[TG_IR.info['reference_mass']]:.2f} ${UNITS['sample_mass']}$"
                )
            ...
            # plotting of fit
            fitting.plot(
                x, data, label="data", lw=2, zorder=num_curves + 1
        ...
        if plot:
            ...
            plt.show()
            fig.savefig(TG_IR.info["name"] + "_" + gas + ".png")

        # slope = pd.Series(frame.index.get_level_values(1)).apply(lambda x: tir.linreg.slope[x]).values
        # intercept = pd.Series(frame.index.get_level_values(1)).apply(lambda x: tir.linreg.slope[x]).values
    ...
    groups = list(
        set([re.split("_", group)[0] for group in peaks.index if group not in gases])
    )
    for group in groups:
        group_set = peaks[["mmol", "mmol_per_mg"]].loc[
            peaks.index.map(lambda x: x.startswith(group))
        ]
        if len(group_set) > 1:
            peaks = peaks.append(
...
def fits(objs, reference, save=True, presets=None, **kwargs):
    ...
    for i, obj in enumerate(objs):
        ...
        for key in results:
            results[key] = results[key].append(
                pd.concat(
                    {sample: pd.DataFrame(peaks[key].rename(num)).T},
                    names=["samples", "run"],
                )
            )
        err = err.append(
            pd.concat({sample: sumsqerr.rename({name: num})}, names=["samples", "run"])
        )
    ...
    for key in results:
        ...
        for sample in samples:
            ...
            if key == "mmol_per_mg":
                ...
                dmmolg_i = (
                    np.power(np.power(lod / mmol, 2) + np.power(dm / g, 2), 0.5)
                    * subset
                )
                ...
                stddev = pd.concat(
                    [
                        pd.DataFrame(dmmol.rename("dev")).T,
                        pd.DataFrame(
                            results[key].loc[sample, drop_cols].std().rename("dev")
                        ).T,
                    ],
                    axis=1,
                )
            ...
            results[key] = results[key].append(
                pd.concat({sample: mean}, names=["samples", "run"])
            )
            results[key] = results[key].append(
                pd.concat({sample: stddev}, names=["samples", "run"])
            )
    ...
    results["mmol"] = check_LODQ(
        results["mmol"], samples, gases, objs, mean="mean", dev="stddev", ref_mass=False
    )
    results["mmol_per_mg"] = check_LODQ(
        results["mmol_per_mg"], samples, gases, objs, mean="mean", dev="dev"
    )
...
def get_presets(path, reference):
    ...
    references = pd.read_excel(
        cfg["fitting_params"], index_col=0, header=[0, 1], sheet_name=None,
    )
...
def check_LODQ(
    ...
    for sample in samples:
        ...
        try:  # for fits() if only one sample is analyzed
            LODQ_test[(sample, ("rel_" + dev))] = (
                LODQ_test.loc[:, (sample, dev)] / LODQ_test.loc[:, (sample, mean)]
            )
        except:
            pass
        try:  # for robustness() only
            LODQ_test[(sample, ("rel_" + meandev))] = (
                LODQ_test.loc[:, (sample, meandev)] / LODQ_test.loc[:, (sample, mean)]
            )
        except:
            pass
        ...
        for limit in ["LOQ", "LOD"]:
            for gas in gases:
                if gas == "CO":
                    list_gas = np.array(
                        list(
                            set(filter(lambda x: gas in x, LODQ_test.index))
                            - set(filter(lambda x: "CO2" in x, LODQ_test.index))
                        )
                    )
                    list_gas = list_gas[
                        list(
                            (
                                (
                                    LODQ_test.loc[list_gas, (sample, mean)]
                                    * ref_mass_mean
                                )
                                < objs[0].stats.loc[gas, ("x_" + limit)]
                            ).values
                        )
                    ]
                    ...
                    list_gas = np.array(
                        list(filter(lambda x: gas in x, LODQ_test.index))
                    )
                    list_gas = list_gas[
                        list(
                            (
