...
class TG_IR:
            ...
            try:
                ...
                print(
                    "IR data found{} for gases {}.".format(
                        " and calibrated (*)" if "linreg" in self.__dict__ else "",
                        ", ".join(
                            [
                                gas
                                + (
                                    "*"
                                    if "linreg" in self.__dict__
    ...
    def plot(self, plot, **kwargs):
        ...
        if plot not in options:
            print(
                "'TG_IR.plot' supports {} as input to plot a respective figure.".format(
                    ", ".join(["'" + option + "'" for option in options])
                )
            )

        if ("ir" not in self.__dict__) and (
            plot in ["IR", "DIR", "cumsum", "IR_to_DTG"]
        ):
    ...
    def fit(
        ...
        **kwargs
        ...
        # fitting
        print(
            "Fitting according to '{}' in Fitting_parameters.xlsx is in progress ...".format(
                reference
            )
        )
        temp = copy.deepcopy(self)
    ...
    def save(self, how="samplelog", **kwargs):
        ...
        if how == "pickle":
            ...
            with pd.ExcelWriter(
                os.path.join(path_output, self.info["name"] + ".xlsx")
            ) as writer:
                try:
                    pd.DataFrame.from_dict(self.info, orient="index").to_excel(
