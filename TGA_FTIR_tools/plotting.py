...
def plot_TGA(
    ...
    if y_axis == "rel":
        ...
        if plot == "sample_mass":
            ...
            ylabel = "{} {} $ {}\,{}^{{-1}}$".format(
                PARAMS["heat_flow"], SEP, UNITS["heat_flow"], UNITS["sample_mass"]
            )

    elif y_axis == "orig":
        y = TG_IR.tga[plot]
        yDTG = TG_IR.tga["dtg"] * 60  # turning dtg from mg/s in mg/min
        ylabelDTG = "{} {} ${}\,min^{{-1}}$".format(
            PARAMS["dtg"], SEP, UNITS["sample_mass"]
        )
        ylabel = "{} {} ${}$".format(PARAMS[plot], SEP, UNITS[plot])

    TGA.set_xlabel(
        "{} {} ${}$".format(PARAMS[x_axis.lower()], SEP, UNITS[x_axis.lower()])
    )
    ...
    if x_axis == "time":
        ...
        temp.plot(
            x,
            TG_IR.tga["sample_temp"],
            label="{} {} ${}$".format(PARAMS["sample_temp"], SEP, UNITS["sample_temp"]),
            ls="dashed",
            color="black",
        )
    ...
    if save:
        ...
        fig.savefig(
            os.path.join(
                path_plots_tga, "{}_TG_{}.png".format(TG_IR.info["name"], y_axis)
            ),
            bbox_inches="tight",
            dpi=DPI,
        )
...
def plot_FTIR(
    ...
    if len(gases) == 0:
        if y_axis == "rel":
            ...
            if len(on_axis - calibrated) != 0:
                print(
                    "{} not calibrated. Proceeding with {}.".format(
                        " and ".join([gas for gas in list(on_axis - calibrated)]),
        ...
        if y_axis == "rel":
            ...
            if len(gases - calibrated) != 0:
                print(
                    "{} not calibrated.".format(
                        " and ".join([gas for gas in (gases - calibrated)])
                    )
                )
            ...
            if len(gases - on_axis) != 0:
                print(
                    "{} not found in IR data.".format(
                        " and ".join([gas for gas in (gases - on_axis)])
                    )
                )
            if len(intersection) != 0:
                print(
    ...
    if y_axis == "orig":
        ...
        graphs[0].set_ylabel(
            "${}\,{}^{{-1}}\,{}^{{-1}}$".format(
                UNITS["molar_amount"], UNITS["sample_mass"], UNITS["time"]
            )
        )
    ...
    for i, gas in enumerate(gases):
        if y_axis == "orig":
            ...
            graphs[i].yaxis.set_minor_locator(
                ticker.AutoMinorLocator()
            )  # switch on minor ticks on each axis
            ...
            tot_mol = (
                (tot_area - TG_IR.linreg["intercept"][gas])
    ...
    graphs[0].xaxis.set_minor_locator(
        ticker.AutoMinorLocator()
    )  # switch on minor ticks on each axis
    ...
    if save:
        ...
        fig.savefig(
            os.path.join(
                path_plots_ir, "{}_IR_{}.png".format(TG_IR.info["name"], y_axis)
            ),
            bbox_inches="tight",
            dpi=DPI,
        )
...
def FTIR_to_DTG(
    ...
    gases = gases_temp
    data = pd.merge(
        TG_IR.tga, TG_IR.ir, how="left", on=["time", "sample_temp"]
    ).dropna()
    ...
    stack.set_ylabel(
        "{}, {} {} ${}\,{}^{{-1}}$".format(
            PARAMS["dtg"],
            ", ".join([get_label(gas) for gas in gases]),
            SEP,
            UNITS["sample_mass"],
            UNITS["time"],
        )
    ...
    if save:
        ...
        fig.savefig(
            os.path.join(path_plot_irdtg, "{}_IRDTG.png".format(TG_IR.info["name"])),
            bbox_inches="tight",
            dpi=DPI,
        )
        out["dtg"] = DTG
        out.to_excel(
            os.path.join(PATHS["dir_output"], TG_IR.info["name"] + "_IRDTG.xlsx")
        )
...
def plots(
    ...
    if plot == "IR":
        ...
        if y_axis == "rel":
            if calibrated == set():
                print(
                    "{} is not calibrated. Change y_axis to 'orig' and rerun command.".format(
                        gas
                    )
                )
                return
    ...
    if plot != "IR":
        ...
        if y_axis == "orig":
            ...
            ax.set_ylabel(
                "{} {} ${}\,{}^{{-1}}$".format(
                    get_label(gas), SEP, UNITS["molar_amount"], UNITS["sample_mass"]
                )
            )
    ...
    for obj in TG_IR_objs:
        ...
        if plot == "TG":
            ...
            ax.plot(
                ...
                label="{}, {}: {:.2f}$\,{}$".format(
                    obj.info["alias"],
                    obj.info["reference_mass"],
                    obj.info[obj.info["reference_mass"]],
                    UNITS["sample_mass"],
                ),
            )
        if plot == "DTG":
